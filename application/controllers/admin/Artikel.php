	...
	public function slider(){
		if ($this->ion_auth->logged_in()) {
			...
			if (!$this->ion_auth->in_group($level)) {
				...
				$data['hasil'] = $this->Admin_m->all_slider();
				$this->load->view('admin/dashboard-v',$data);
			}
		}else{
			$pesan = 'Login terlebih dahulu';
		...
		if ($this->ion_auth->logged_in()) {
			...
			if (!$this->ion_auth->in_group($level)) {
				...
				$data = array(
					...
					'alias_slider' => strtolower(url_title($post['judul_slider']))
					);
				if (!empty($_FILES["img_slider"]["tmp_name"])) {
					$config['file_name'] = strtolower(url_title('slider'.'-'.$post['judul_slider'].'-'.date('Y-m-d').'-'.time('H-i-s')));
					$config['upload_path'] = './asset/img/artikel/';
					$config['allowed_types'] = 'gif|jpg|png';
					$config['max_size'] = 2048;
					$config['max_width'] = '';
					...
					if (!$this->upload->do_upload('img_slider')){
						$error = $this->upload->display_errors();
						$this->session->set_flashdata('errors', $error );
						redirect(base_url('index.php/admin/artikel/slider'));
					}
					else{
						$img = $this->upload->data('file_name');
						$data['img_slider'] = $img;
				...
				$this->Admin_m->Insert_slider($data);
				$pesan = 'Slider '.$post['judul_slider'].' Berhasil dibuat';
				$this->session->set_flashdata('message', $pesan );
				redirect(base_url('index.php/admin/artikel/slider'));
			}
		}else{
	...
	public function edit_slider($id){
		if ($this->ion_auth->logged_in()) {
			...
			if (!$this->ion_auth->in_group($level)) {
				...
				$data['title'] = 'Edit - '.$this->Admin_m->detail_slider($id)->judul_slider;
				$data['infopt'] = $this->Admin_m->info_pt(1);
				$data['users'] = $this->ion_auth->user()->row();
				$data['aside'] = 'nav/nav-members';
				$data['detail'] = $this->Admin_m->detail_slider($id);
			...
			if (!$this->ion_auth->in_group($level)) {
				...
				if (!empty($_FILES["img_slider"]["tmp_name"])) {
					...
					if (!$this->upload->do_upload('img_slider')){
						$error = $this->upload->display_errors();
						$this->session->set_flashdata('errors', $error );
						redirect(base_url('index.php/admin/artikel/edit_slider/'.$id));
					}
					else{
						$file = $this->Admin_m->detail_slider($id)->img_slider;
						if ($file != "default.jpg") {
							unlink("asset/img/artikel/$file");
						}
						$img = $this->upload->data('file_name');
				...
				$this->Admin_m->update_slider($id,$data);
				$pesan = 'slider '.$post['judul_slider'].' Berhasil diedit';
				$this->session->set_flashdata('message', $pesan );
				redirect(base_url('index.php/admin/artikel/slider'));
			}
		}else{
